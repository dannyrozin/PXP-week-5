...
void draw() {
  ...
  int blurAmount = 4;                        // change this to make the effect more pronounced
  ...
  for (int x = max(mouseX-100, blurAmount); x<min(mouseX+100, width-blurAmount); x++) {     // looping 100 pixels around the mouse, we have to make sure we wont 
    for (int y = max(mouseY-100, blurAmount); y<min(mouseY+100, height-blurAmount); y++) {  // be accessing pixels outside the bounds of our array
     if (dist(mouseX, mouseY, x, y)< 100) {                       // lets just do a circle radius 100
        ...
        PxPGetPixel(x, y, ourVideo.pixels, width);             // Up to here everything was the same as blur, which are the low frequency
                                                               // features, but we want to keep the high freequency, so we subtract the blured from  
                                                               // the original RGB, and add 128 so its not minus
        PxPSetPixel(x, y, R-sumR+128, G-sumG+128, B-sumB+128, 255, pixels, width);    // sets the R,G,B values to the window
      }
    }
  }
